...
export default function CartPage() {
  ...
  const handleWhatsApp = () => {
    window.open(buildWhatsAppLink(items, '5491100000000'), '_blank')
  }
  ...
  if (items.length === 0) {
    return (
      <div className="bg-[#0d0d0d] min-h-screen flex flex-col items-center justify-center gap-6 px-4">
        ...
        <p className="text-gray-400">Explora nuestros productos y encontra el equipo ideal.</p>
        <Link
          to="/productos"
    ...
    <div className="bg-[#0d0d0d] min-h-screen py-12 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={clearCart} className="text-gray-500 hover:text-red-400 text-sm transition-colors">
            Vaciar carrito
          </button>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.product.id} className="bg-gray-900 rounded-lg p-4 flex gap-4">
                <img
                  src={item.product.images[0] ?? '/assets/images/hero/12_KELLER_BCA_2023_00238.jpg'}
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex items-center border border-gray-700 rounded">
                    <button onClick={() => updateQty(item.product.id, item.quantity - 1)} className="px-2 py-1 text-white hover:bg-gray-800 transition-colors text-sm">-</button>
                    <span className="px-3 py-1 text-white text-sm border-x border-gray-700">{item.quantity}</span>
                    <button onClick={() => updateQty(item.product.id, item.quantity + 1)} className="px-2 py-1 text-white hover:bg-gray-800 transition-colors text-sm">+</button>
                  </div>
                  <button onClick={() => removeItem(item.product.id)} className="text-gray-500 hover:text-red-400 text-xs transition-colors">
                    Eliminar
                  </button>
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-gray-900 rounded-lg p-6 sticky top-24">
              ...
              <button
                ...
                className="w-full text-white py-4 rounded font-bold flex items-center justify-center gap-3 hover:opacity-90 transition-opacity"
